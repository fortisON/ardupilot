...
#if MODE_GUIDED_NOGPS_ENABLED
/*
  fork-specific conversion table: maps old top-level parameters to their
  new names so saved values carry over on first boot after an upgrade
 */
static const AP_Param::ConversionInfo fork_conversion_table[] = {
...
#if MODE_GUIDED_NOGPS_ENABLED
    AP_Param::convert_old_parameters(&fork_conversion_table[0], ARRAY_SIZE(fork_conversion_table));
#endif

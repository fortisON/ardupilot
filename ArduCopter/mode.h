...
private:
    ...
    // fractional accumulator for incremental mode (sub-degree remainder)
...
public:
    ...
    // fork: RAM-only / persistent setters so GuidedNoGPS RC input can adjust RTL_ALT_M
    void set_altitude_m(float alt_m) { altitude_m.set(alt_m); }
    void save_altitude_m() { altitude_m.save(); }
    void set_and_save_altitude_m_ifchanged(float alt_m) { altitude_m.set_and_save_ifchanged(alt_m); }
